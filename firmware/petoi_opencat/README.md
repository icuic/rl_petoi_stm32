...
- `rl_ble_bridge_v0.h/.cpp`：预演 OpenCatEsp32 BLE client notify callback 的 binary-safe `Y + RL frame`
  接入口；输入保留 OpenCat token，响应保持裸 `RL frame`，以匹配 STM32 transport reader。
...
- `tests/rl_ble_bridge_v0_test.cpp`：验证 BLE notify 分片输入、文本命令 fallback 和裸 `RL` 响应。
